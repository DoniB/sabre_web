  ...
  <dashboard :title="title">
    <md-table v-model="users" md-card>
      <md-table-toolbar>
        ...
        <md-button @input="searchByName" class="md-icon-button"
      ...
      >
        ...
        <md-table-cell md-label="Cadastrado">{{
          item.created_at | toLocalTime
        }}</md-table-cell>
        <md-table-cell md-label="Atualizado">{{
          item.updated_at | toLocalTime
        }}</md-table-cell>
        <md-table-cell md-label="Adm">{{
        ...
        <md-table-cell>
          ...
          <div @click.stop="showUserActiveStateSwapDialog(item)">
            <md-icon>{{
              item.active ? 'block' : 'add_circle_outline'
            }}</md-icon>
          </div>
        </md-table-cell>
      </md-table-row>
    ...
    <md-dialog :md-active.sync="showUserStateDialog">
      <md-dialog-title
        >Alterar usuário
        {{ currentUserForChangeState.username }}?</md-dialog-title
      >
      <div class="dialog-content">
        <div v-if="currentUserForChangeState.active">
          Ao confirmar esta ação não será mais possivel efetuar login com esta
          conta
        </div>
        <div v-else>
          Ao confirmar esta ação esta conta estará apta a efetuar login
          novamente
        </div>
      </div>
      <md-dialog-actions>
        <md-button @click="updateUserActiveState" class="md-primary">{{
          currentUserForChangeState.active ? 'Desativar' : 'Ativar'
        }}</md-button>
        <md-button @click="showUserStateDialog = false" class="md-primary"
          >Cancelar</md-button
        >
      </md-dialog-actions>
    </md-dialog>
